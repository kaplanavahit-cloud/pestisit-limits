...
export default function DecisionSupportPage() {
    ...
    const handleAnalyze = async () => {
        ...
        for (let i = 0; i < parsed.length; i++) {
            ...
            // 🔵 TÜRKİYE YASAKLI LİSTESİ
            if (isTurkeyBanned) {
                evalResults.push({
                    id: `${Date.now()}-${i}-tr-banned`,
                    product: row.product,
                    pesticide: row.pesticide,
                    measured: row.reportedLimit,
                    country: { id: 0, name: 'Türkiye', code: 'TR' },
                    limit: 0,
            ...
            // 🔴 RUSYA ZERO TOLERANCE LİSTESİ
            if (isRussiaZero) {
                evalResults.push({
                    id: `${Date.now()}-${i}-ru-zero`,
                    product: row.product,
                    pesticide: row.pesticide,
                    measured: row.reportedLimit,
            ...
            // 🔵 RUSYA LİMİTSİZ LİSTESİ
            if (isRussiaNoLimit) {
                evalResults.push({
                    id: `${Date.now()}-${i}-ru-nolimit`,
                    product: row.product,
                    pesticide: row.pesticide,
                    measured: row.reportedLimit,
                    country: { id: 0, name: 'Rusya', code: 'RU' },
                    ...
                    isTurkeyBanned,
                    isRussiaZero,
                    isRussiaNoLimit: true,
                });
            }

            const limits = await searchLimits(row.product, row.pesticide);
            const hasRussiaLimit = limits.some(l => l.countriesa?.code === 'RU');

            if (!limits.length) {
                if (!isTurkeyBanned && !isRussiaZero && !isRussiaNoLimit) {
                    evalResults.push({
            ...
            for (let idx = 0; idx < limits.length; idx++) {
                ...
                // Özel listeler için zaten satır eklediysek atla
                if (isTurkey && isTurkeyBanned) continue;
                if (isRussia && isRussiaZero) continue;
                if (isRussia && isRussiaNoLimit) continue;
                ...
                if (applyOB) {
                    ...
                    adjustedValue = row.reportedLimit * (1 - obPct / 100);
                }
            ...
            // Rusya için hiç limit yoksa ve listede de yoksa
            if (!hasRussiaLimit && !isRussiaZero && !isRussiaNoLimit) {
                evalResults.push({
                    id: `${Date.now()}-${i}-ru-missing`,
                    product: row.product,
                    pesticide: row.pesticide,
                    measured: row.reportedLimit,
                    country: { id: 0, name: 'Rusya', code: 'RU' },
    ...
    const tabClass = (t: string) =>
        `px-4 py-2 text-sm font-medium rounded-t-xl transition-all duration-200 ${activeTab === t
        ...
        <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-emerald-50">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
                ...
                <div className="flex gap-1 mb-0 border-b border-gray-200">
                    <button className={tabClass('input')} onClick={() => setActiveTab('input')}>📋 Veri Girişi</button>
                    <button className={tabClass('report')} onClick={() => setActiveTab('report')} disabled={allResults.length === 0}>
                    ...
                    <button className={tabClass('special')} onClick={() => setActiveTab('special')}>🔖 Özel Listeler</button>
                </div>
                ...
                {activeTab === 'input' && (
                    <div className="pt-6 space-y-5">
                        <div className="bg-white rounded-2xl border border-gray-100 p-5 shadow-sm">
                            <div className="flex flex-wrap items-center gap-5">
                                ...
                                <label className="flex items-center gap-2 cursor-pointer text-sm text-gray-600">
                                    <input type="checkbox" checked={useUncertainty} onChange={e => setUseUncertainty(e.target.checked)} className="w-4 h-4 rounded text-emerald-500 border-gray-300 focus:ring-emerald-200" />
                                    Aktif
                                </label>
                                {useUncertainty && (
                                    <div className="flex items-center gap-2">
                                        <input type="range" min="10" max="100" step="5" value={measurementUncertainty} onChange={e => setMeasurementUncertainty(Number(e.target.value))} className="w-28 accent-emerald-500" />
                        ...
                        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                            ...
                            <div className="p-6">
                                <textarea value={pasteData} onChange={e => setPasteData(e.target.value)} placeholder={`Örnek format (3 sütun: Ürün | Pestisit | Değer):
...
💡 ÖB, sonucun ALT limitine uygulanır (Ölçülen - ÖB). CODEX ve Rusya'ya uygulanmaz.`} className="w-full h-44 px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-gray-900 placeholder-gray-400 font-mono text-sm focus:outline-none focus:border-emerald-400 focus:ring-2 focus:ring-emerald-100 transition-all resize-none" />
                                ...
                                <div className="flex gap-3 mt-5">
                                    <button onClick={handleAnalyze} disabled={loading || !pasteData.trim()} className="flex-1 bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-600 hover:to-teal-600 text-white font-medium py-3 rounded-xl transition-all duration-200 shadow-md shadow-emerald-200 disabled:opacity-50 disabled:cursor-not-allowed text-sm">
                                        {loading ? <span className="flex items-center justify-center gap-2"><svg className="animate-spin h-4 w-4" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" /></svg> Analiz Ediliyor...</span> : <span className="flex items-center justify-center gap-2">⚡ Analizi Başlat</span>}
                                    </button>
                                    <button onClick={clearAll} className="px-6 bg-gray-100 hover:bg-gray-200 text-gray-700 font-medium py-3 rounded-xl transition-all duration-200 text-sm">Temizle</button>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="pt-6 space-y-5">
                        ...
                        {hiddenCount > 0 && (
                            <div className="flex justify-end">
                                <button onClick={unhideAll} className="flex items-center gap-1 px-3 py-1.5 bg-gray-100 hover:bg-gray-200 text-gray-600 text-xs rounded-lg transition-colors">🔄 Tüm gizlenenleri göster ({hiddenCount})</button>
                            </div>
                        )}
                        ...
                        {Array.from(groupedResults.entries()).map(([key, results]) => {
                            ...
                            return (
                                <div key={key} className="bg-white rounded-2xl border border-gray-100 overflow-hidden shadow-sm">
                                    <div className={`px-5 py-4 border-b flex items-center gap-3 flex-wrap ${hasFail ? 'bg-red-50 border-red-100' : hasWarn ? 'bg-amber-50 border-amber-100' : 'bg-emerald-50 border-emerald-100'}`}>
                                        ...
                                        <div className="flex flex-wrap gap-1.5">
                                            {flags.map(f => (
                                                <span key={f.type} className={`text-xs px-2 py-0.5 rounded-full font-medium ${f.type === 'turkey-banned' ? 'bg-orange-100 text-orange-700' : f.type === 'russia-zero' ? 'bg-red-100 text-red-700' : 'bg-purple-100 text-purple-700'}`}>{f.label}</span>
                                            ))}
                                        </div>
                                    </div>
                                        ...
                                        <table className="w-full text-sm">
                                            ...
                                            <tbody className="divide-y divide-gray-100">
                                                {results.map((r) => {
                                                    ...
                                                    const barColor = r.status === 'PASS' ? '#10b981' : r.status === 'WARNING' ? '#f59e0b' : '#ef4444';

                                                    return (
                                                        <tr key={r.id} className={`transition-colors hover:bg-gray-50 ${['FAIL', 'BANNED', 'ZERO_TOLERANCE'].includes(r.status) ? 'bg-red-50/40' : r.status === 'WARNING' ? 'bg-amber-50/40' : ''}`}>
                                                            <td className="px-3 py-3">
                                                                <button onClick={() => hideResult(r.id)} className="w-6 h-6 rounded-full flex items-center justify-center text-gray-400 hover:bg-red-100 hover:text-red-600 transition-colors" title="Bu sonucu gizle (veritabanından silmez)">✕</button>
                                                            </td>
                                                            ...
                                                            <td className="px-5 py-3 font-mono text-gray-700">
                                                                {r.isRussiaNoLimit ? (
                                                                    <span className="text-purple-600 font-semibold">Limitsiz</span>
                                                                ) : r.isRussiaZero ? (
                                                            ...
                                                            <td className="px-5 py-3 font-mono text-gray-700">{r.measured.toFixed(3)}</td>
                                                            ...
                                                            <td className="px-5 py-3"><span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-semibold ${cfg.bg} ${cfg.text}`}>{cfg.icon} {cfg.label}</span></td>
                                                            <td className="px-5 py-3">
                                                                {r.status !== 'NOT_FOUND' && r.limit > 0 ? (
                                                                    <div className="flex items-center gap-2 min-w-[100px]">
                                                                        <div className="flex-1 bg-gray-200 rounded-full h-1.5">
                                                                            <div className="h-1.5 rounded-full transition-all duration-300" style={{ width: `${Math.min(ratioPercent, 100)}%`, backgroundColor: barColor }} />
                                                                        </div>
                                                                        <span className="text-xs text-gray-500 w-10 text-right font-mono">{ratioPercent.toFixed(0)}%</span>
                                                                    </div>
                        ...
                        <div className="flex gap-3 justify-center pt-2">
                            <button onClick={() => setActiveTab('input')} className="flex items-center gap-2 px-5 py-2.5 bg-white border border-gray-200 text-gray-600 hover:text-emerald-600 hover:border-emerald-300 rounded-xl text-sm transition-all duration-200 shadow-sm">🔄 Yeni Analiz</button>
                            <button onClick={exportCSV} className="flex items-center gap-2 px-5 py-2.5 bg-white border border-gray-200 text-gray-600 hover:text-emerald-600 hover:border-emerald-300 rounded-xl text-sm transition-all duration-200 shadow-sm">⬇ CSV İndir</button>
                        </div>
                    </div>
                )}
                    ...
                    <div className="pt-6 space-y-5">
                        <div className="grid grid-cols-3 gap-3">
                            {[
                                ...
                                <button key={item.key} onClick={() => { setSpecialListTab(item.key); setSpecialSearch(''); }} className={`p-4 rounded-2xl border text-left transition-all duration-200 ${specialListTab === item.key ? item.color === 'orange' ? 'bg-orange-50 border-orange-300' : item.color === 'red' ? 'bg-red-50 border-red-300' : 'bg-purple-50 border-purple-300' : 'bg-white border-gray-200 hover:border-gray-300'}`}>
                                    <div className="text-2xl mb-1">{item.icon}</div>
                                    <div className="font-semibold text-sm text-gray-800">{item.label}</div>
                                    <div className="text-xs text-gray-400 mt-0.5">{loadingLists ? 'Yükleniyor...' : `${item.count} madde`}</div>
                                </button>
                            ))}
                        </div>

                        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                            <div className={`px-5 py-4 border-b ${specialListTab === 'turkey-banned' ? 'bg-orange-50 border-orange-100' : specialListTab === 'russia-zero' ? 'bg-red-50 border-red-100' : 'bg-purple-50 border-purple-100'}`}>
                                <p className="text-sm text-gray-600">{currentSpecialList.description}</p>
                            </div>
                            <div className="p-5">
                                <input type="text" value={specialSearch} onChange={e => setSpecialSearch(e.target.value)} placeholder="Pestisit adı ara..." className="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm text-gray-900 placeholder-gray-400 focus:outline-none focus:border-emerald-400 focus:ring-2 focus:ring-emerald-100 transition-all mb-4" />
                                {loadingLists ? (
                                    ...
                                    <div className="text-center py-10 text-gray-400 text-sm">{specialSearch ? 'Eşleşme bulunamadı' : 'Liste boş veya yüklenemedi'}</div>
                                ) : (
                                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2 max-h-96 overflow-y-auto">
                                        {filteredSpecialList.map((item, idx) => (
                                            <div key={idx} className={`flex items-center gap-2 px-3 py-2 rounded-lg text-sm border ${specialListTab === 'turkey-banned' ? 'bg-orange-50 border-orange-100 text-orange-800' : specialListTab === 'russia-zero' ? 'bg-red-50 border-red-100 text-red-800' : 'bg-purple-50 border-purple-100 text-purple-800'}`}>
                                ...
                                <p className="text-xs text-gray-400 mt-3">{filteredSpecialList.length} / {currentSpecialList.items.length} madde gösteriliyor</p>
                            </div>
                        </div>
                    </div>
                )}
